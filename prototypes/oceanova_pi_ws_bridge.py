...
Ports:
  8765  control + telemetry (JSON text)
  8766  video (binary JPEG frames)

Separate sockets keep video traffic from delaying control commands.
"""
...
class CameraWorker(threading.Thread):
    ...
    def snapshot(self):
        with self.lock:
            return (self.sequence, self.latest_jpeg, self.online, self.error,
                    self.last_frame_monotonic, self.actual_width,
                    self.actual_height, self.actual_fps)
...
def parse_telemetry(line: str) -> Optional[dict]:
    ...
    if len(fields) != 10 or fields[0] != "T":
        return None
    try:
        return {"type": "telemetry", "esp32_time_ms": int(fields[1]),
                "yaw": float(fields[2]), "pitch": float(fields[3]),
                "roll": float(fields[4]), "left_motor": int(fields[5]),
                "right_motor": int(fields[6]), "armed": bool(int(fields[7])),
...
class SerialBridge(threading.Thread):
    ...
    def status_snapshot(self) -> dict:
        with self.state_lock:
            ...
            return {"online": self.online, "device": self.device, "error": self.error,
                    "telemetry_age_ms": age_ms, "last_line": self.latest_serial_line}
    ...
    def _connect(self) -> Optional[serial.Serial]:
        ...
        for device in devices:
            try:
                ...
                port = serial.Serial(port=device, baudrate=SERIAL_BAUD, timeout=0.1,
...
async def control_handler(websocket, serial_bridge: SerialBridge, camera: CameraWorker, control_lock: asyncio.Lock):
    ...
    if control_lock.locked():
        await websocket.send(json.dumps({"type": "error", "message": "Another control client is already connected"}))
        await websocket.close(code=1013, reason="Control client already connected")
        return
    async with control_lock:
        session = ControlSession()
        serial_bridge.emergency_stop()
        await websocket.send(json.dumps({"type": "hello", "service": "oceanova-crawler",
                                         "control_port": CONTROL_PORT, "video_port": VIDEO_PORT,
            ...
            while True:
                ...
                if now - last_system_send >= SYSTEM_STATUS_PERIOD_SECONDS:
                    ...
                    await websocket.send(json.dumps({"type": "system",
                                                      ...
                                                      "camera_fps": camera_state[7]}, separators=(",", ":")))
            ...
            while True:
                if session.armed_session:
                    ...
                    if age > TOPSIDE_WATCHDOG_SECONDS:
                        ...
                        try:
                            await websocket.send(json.dumps({"type": "fault", "reason": "topside_command_timeout"}))
                        except Exception:
                            return
        ...
        try:
            async for message in websocket:
                ...
                if msg_type == "ping":
                    await websocket.send(json.dumps({"type": "pong", "client_time": command.get("client_time"), "server_time": time.time()}))
                elif msg_type == "stop":
                    serial_bridge.emergency_stop()
                    session.armed_session = False
...
async def video_handler(websocket, camera: CameraWorker):
    ...
    await websocket.send(json.dumps({"type": "video_info", "format": "jpeg",
...
async def async_main() -> None:
    ...
    print("\nOceanova crawler WebSocket bridge")
